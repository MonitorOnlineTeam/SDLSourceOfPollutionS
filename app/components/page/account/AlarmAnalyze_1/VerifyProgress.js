...
class VerifyProgress extends PureComponent {
    ...
    this.props.dispatch(
      createAction('alarmAnaly/GetWarningVerifyCheckInfo')({
        ...
        callback: result => {
          ...
          let appResult = SentencedToEmpty(
            this.props.WarningVerifyCheckInfo,
            ['data', 'Datas', 'appResult'],
            [],
          );
          // console.log('appResult = ', appResult);
          appResult.map((item1, key1) => {
            processingProgressImageItem = [];
            SentencedToEmpty(item1, ['FileList'], []).map((item2, index2) => {
              tempArr = item2.split('/');
              fileName = tempArr[tempArr.length - 1];
      ...
      rtnVal.push(
        <View
          ...
          <TouchableOpacity
            onPress={() => {
              // this.setState({ modalVisible: true, showImage: { url: source.uri } });
              // console.log(SentencedToEmpty(this.state, ['processingProgressImages', idx], []));
              this.setState({
                modalVisible: true,
                largeImageIndex: key,
                showImage: SentencedToEmpty(
                  this.state,
                  ['processingProgressImages', idx],
                  [],
                ),
              });
            }}
  ...
  getRectificationRecordName = () => {
    let IsRectificationRecord = SentencedToEmpty(
      this.props.WarningVerifyCheckInfo,
      ['data', 'Datas', 'checkInfo', 'IsRectificationRecord'],
      0,
    );

...
class AlarmDetails extends PureComponent {
        ...
        alertContent = [
            {
                text: '数据列表',
                onPress: () => {
                    let describe = SentencedToEmpty(this.props.alarmChartData, ['data', 'Datas', 'describe'], '');
                    ...
                    let rtnFinal = SentencedToEmpty(this.props.alarmChartData, ['data', 'Datas', 'rtnFinal', 0, 'Data'], []);
                    ...
                    this.props.dispatch(
                        NavigationActions.navigate({
                            ...
                            params: { ...this.alarmObj, type: 'AlarmDataChart', timeRange: arr1.length > 0 ? arr1 : arr }
                        })
                    );
                }
            },
    ...
    statusPageOnRefresh = () => {
        this.props.dispatch(
            createAction('alarmAnaly/getSingleWarning')({
                modelWarningGuid: this.alarmObj.ID
            })
        );
    };
    ...
    getOption = (item, idx) => {
        ...
        SentencedToEmpty(item, ['data'], []).map((subI, subId) => {
            if (showFlag == 5) {
                    ...
                    markLineArr.push([
                        {
                            ...
                            coord: [moment(split.startTime).format('MM-DD HH:mm'), split.LowLimit]
                        },
                        {
                            coord: [moment(split.endTime).format('MM-DD HH:mm'), split.LowLimit]
                        }
                    ]);
                    markLineArr.push([
                        {
                            ...
                            name: '波动上限',
                            coord: [moment(split.startTime).format('MM-DD HH:mm'), split.UpperLimit]
                        },
                        {
                            coord: [moment(split.endTime).format('MM-DD HH:mm'), split.UpperLimit]
                        }
                    ]);
                });
                ...
                //设置上限，超过预设值使用随机数处理
                markLineArr.push(
                    { yAxis: subI.defaultValue, lineStyle: { color: colors[idx] }, name: '预设值' },
                    {
                ...
                markLineArr.push({ yAxis: subI.monitorValue, name: '值', lineStyle: { color: colors[idx] } });
            } else {
                markLineArr.push([
                    { yAxis: subI.standardLower || 'min', lineStyle: { color: colors[idx] }, name: '下限值' },
                    {
            ...
            if (this.props.showDataSort[idx] == subId) {
                ...
                if (showFlag == -1) {
                    ...
                    subI.data.map((subData, j) => {
                        newChartData.push({ value: subData, symbol: 'triangle', symbolSize: 20, workingFlagName: subI.workingFlagName[j], dataFlagName: subI.dataFlagName[j], unit: subI.unit });
                    });
                    ...
                    subI.data.map((subData, j) => {
                        if (
                            ...
                            newChartData.push({ value: subData, showSymbol: false, symbolSize: 0, workingFlagName: subI.workingFlagName[j], dataFlagName: subI.dataFlagName[j], unit: subI.unit });
                        }
                    });
                } else {
                    newChartData = subI.data;
                ...
                series.push({
                    ...
                    markLine: {
                        label: {
                            formatter: showFlag == 5 ? '{b}' : showFlag == 2 || showFlag == 3 ? '{b}: {c}' : '{c}'
                        },
                        data: showFlag == 1 || showFlag == -1 ? markLineArr[subId] : markLineArr
                    }
                });
            }
        });
        let option = {
            ...
            color: colors[idx],
            grid: { left: '10%', top: '12%', width: showFlag == 5 ? '78%' : showFlag == 2 || showFlag == 3 ? '67%' : '78%', height: '78%' },
                        ...
                        <View style={{ alignItems: 'center', marginTop: 10 }}>
                            ...
                            <View style={{ flexDirection: 'row', flexWrap: 'nowrap', minHeight: 40, alignItems: 'center' }}>
                                ...
                                {item.PollutantCode == 'Error' ? (
                                    <Text style={[styles.content, { color: 'red', fontSize: 17, textAlign: 'center', width: 100 }]}>{Data[0][item.PollutantCode]}</Text>
                                ) : (
                                    Data.map(subI => {
    ...
    renderChart() {
        ...
        SentencedToEmpty(chartData, ['chartData'], []).map((item, idx) => {
            {
                typeCode == 'c0af25fb-220b-45c6-a3de-f6c8142de8f1' || typeCode == 'ab2bf5ec-3ade-43fc-a720-c8fd92ede402' || typeCode == 'f021147d-e7c6-4c1d-9634-1d814ff9880a'
                    ? views.push(
                    ...
                    : views.push(
                        <View style={{ marginTop: 10 }}>
                            <SDLText style={[styles.content, { color: '#333', fontSize: 15 }]}>{item.title}</SDLText>
                                    ...
                                    return (
                                        <TouchableOpacity
                                            onPress={() => {
                                                ...
                                                newSort[idx] = subid;
                                                this.props.dispatch(createAction('alarmAnaly/updateState')({ showDataSort: newSort }));
                                            }}
                                            style={{ backgroundColor: '#F4F7FF', borderRadius: 13, height: 25, alignItems: 'center', justifyContent: 'center', paddingLeft: 5, paddingRight: 5, marginRight: 5 }}
                                        >
                                            <Text style={{ color: this.props.showDataSort[idx] == subid ? '#4AA0FF' : '#999999' }}>{subI.pollutantName}</Text>
                                        </TouchableOpacity>
                                    );
                                })}
                            </ScrollView>
                            ...
                            <View style={{ height: 180, backgroundColor: '#ffffff' }}>
                                <Echarts option={this.getOption(item, idx)} height={180} flex={1} />
                            </View>
                            {//借用本设备合格历史数据
                                typeCode == 'd5dea4cc-bd6c-44fa-a122-a1f44514b465' ? (
                                    <View style={{ height: 180, marginTop: 10, backgroundColor: '#ffffff' }}>
                                        <Echarts option={this.getHistoryOption(item, idx)} height={180} flex={1} />
                                    </View>
                                ) : null}
                        </View>
                    );
    ...
    render() {
        console.log('alramButtonList = ', this.props.alramButtonList);
        var options = {
            hiddenTitle: true,
            innersHeight: 300
        };
        ...
        let showChart = true;
        if (
            SentencedToEmpty(data, ['WarningTypeCode'], '') == '3d209ce2-da92-44c4-916b-8874d05da558' ||
            SentencedToEmpty(data, ['WarningTypeCode'], '') == 'ce61d9a9-5d0d-4b66-abbd-72a89e823ee2' ||
            SentencedToEmpty(data, ['WarningTypeCode'], '') == 'c934b575-a357-4a2c-b493-02849ce9cee3' ||
            SentencedToEmpty(data, ['WarningTypeCode'], '') == 'a59cce2a-8558-4c42-8a45-4d8402e4b29d' ||
            SentencedToEmpty(data, ['WarningTypeCode'], '') == '0fa091a3-7a19-4c9e-91bd-c5a4bf2e9827' ||
            SentencedToEmpty(data, ['WarningTypeCode'], '') == '178fd470-ca31-480a-835a-3322fd57a4f0' ||
            SentencedToEmpty(data, ['WarningTypeCode'], '') == 'b9601a0f-22af-4a07-927f-82d6369f2e12'
        ) {
            showChart = false;
        }
        return (
            ...
            >
                <KeyboardAwareScrollView showsVerticalScrollIndicator={false} ref="scroll" style={{ flex: 1, width: SCREEN_WIDTH, padding: 13 }}>
                    ...
                    <View style={[styles.card, { marginTop: 13, marginBottom: 20 }]}>
                        ...
                        <View style={{ backgroundColor: '#F3F3F3', width: '100%', marginTop: 15, height: 1 }}></View>
                        <SDLText style={[{ flex: 1, marginTop: 10, color: globalcolor.recordde, fontSize: 15, lineHeight: 18 }]}>{`${SentencedToEmpty(chartData, ['describe'], '')}`}</SDLText>
                            ...
                            onPress={() => {
                                this.props.dispatch(
                                    NavigationActions.navigate({
                                        ...
                                        params: [{ ...this.alarmObj, ModelWarningGuid: this.alarmObj.ID }]
                                    })
                                );
                            }}

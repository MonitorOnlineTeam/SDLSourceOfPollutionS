...
export default class AlarmDataChart extends React.Component {
    ...
    constructor(props) {
        let timeRange = props.navigation.state.params.timeRange;
        super(props);
        console.log('数据列表1.0');
        this.state = {
    ...
    getLinearCoefficientOption = () => {
        ...
        self.props.navigation.state.params.timeRange.map(item => {
            if (
                ChartArr.length > 0 &&
                Date.parse(item.replace(/-/g, '/')) - Date.parse(ChartArr[0]['MonitorTime'].replace(/-/g, '/')) > 0 &&
                Date.parse(item.replace(/-/g, '/')) - Date.parse(ChartArr[ChartArr.length - 1]['MonitorTime'].replace(/-/g, '/')) < 0
            ) {
                markLineArr.push({ name: '发现异常', xAxis: moment(item).format('YYYY-MM-DD HH:mm') });
            }
        });
        ...
        PollutantArr.map(function (item, idx) {
            ...
            if (selectIndex < 0) {
                ...
                ChartArr.map(function (subitem) {
                    data.push({ value: subitem[item.PollutantCode], workingFlagName: subitem['WorkCon'], dataFlagName: subitem[item.PollutantCode + '_Flag'] || '-', unit: '8' });
                });
                    ...
                    {this.props.AlarmDataChartValue.status == 200 ? (
                        <View style={{ flex: 1 }} horizontal={true} showsHorizontalScrollIndicator={false}>
                            <View style={{ flexDirection: 'row', flexWrap: 'wrap', backgroundColor: '#fff', paddingRight: 100, alignItems: 'center' }}>
                                {SentencedToEmpty(this.props.AlarmChartPollutant, ['data', 'Datas'], []).map((item, idx) => {
                                    return (
                                        <TouchableOpacity
                                            onPress={() => {
                                                let nowSelect = SentencedToEmpty(this.props.AlarmChartPollutant, ['pollttantSelect'], []);
                                                ...
                                                if (
                                                    ...
                                                    nowSelect.splice(
                                                        SentencedToEmpty(this.props.AlarmChartPollutant, ['pollttantSelect'], []).findIndex(subI => {
                                                            if (subI.PollutantName == item.PollutantName) return true;
                                                        }),
                                                        1
                                                    );
                            ...
                            <View style={{ flexDirection: 'row', flexWrap: 'wrap', backgroundColor: '#fff', paddingRight: 100, alignItems: 'center' }}>
                                ...
                                {['两周', '一月', '三月'].map((item, idx) => {
                                    return (
                                        <TouchableOpacity
                                            onPress={() => {
                                                let timeRange = this.props.navigation.state.params.timeRange;
                                                let selectTime = [];
                                                if (item == '两周') {
                                                    selectTime = [
                                                        moment(timeRange[timeRange.length - 1])
                                        ...
                                        >
                                            <Text style={{ color: this.state.selectDateRange == item ? '#ee6666' : '#333' }}>{item}</Text>
                                        </TouchableOpacity>
                                    );
                                })}

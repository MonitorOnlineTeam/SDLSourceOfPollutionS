...
export default class AlarmAnalyList extends PureComponent {
    static navigationOptions = ({ navigation }) => {
        if (typeof navigation.state.params.headerRight != 'undefined' && navigation.state.params.headerRight) {
    ...
    constructor(props) {
        ...
        let currentItem = SentencedToEmpty(this.props.navigation, ['state', 'params', 'item'], {});
                                    ...
                                    <Touchable onPress={() => this._onItemClick(item)}>
                                        <View style={[{ width: SCREEN_WIDTH, minHeight: 123, flexDirection: 'column', justifyContent: 'space-between', backgroundColor: '#fff' }]}>
                                            <View style={[{ height: 45, alignItems: 'center', flexDirection: 'row', paddingStart: 13 }]}>
                                                    ...
                                                    item.IsCheck == 0 && (
                                                        <TouchableOpacity
                                                            onPress={() => {
                                                                ...
                                                                item.onRefresh = this.onRefresh;
                                                                let newSelectIndex = newSelect.findIndex(sitem => sitem.ID == item.ID);
                                                                if (newSelectIndex > -1) {
                                                        ...
                                                        >
                                                            <Image
                                                                ...
                                                                source={this.state.selectItems.findIndex(sitem => sitem.ID == item.ID) > -1 ? require('../../../images/auditselect.png') : require('../../../images/alarm_unselect.png')}
                                                            />
                                                        </TouchableOpacity>
                                                    )}
                                                {item.IsCheck == 1 && naviItem.status == 1 && <Image style={{ width: 25, height: 25, marginRight: 3 }} source={require('../../../images/chehui.png')} />}
                                                <Text numberOfLines={1} style={[{ color: '#333', textAlign: 'center', maxWidth: SCREEN_WIDTH - 150, lineHight: 45 }]}>{`${item.ParentName || ''}-${item.PointName || ''}`}</Text>
                                                <View
                                                    style={{
                                                        alignItems: 'center',
                                                        justifyContent: 'center',
                                                        borderRadius: 5,
                                                        backgroundColor: item.CheckResult == '未核实' ? 'rgba(102,102,102,0.1)' : item.CheckResult == '有异常' ? 'rgba(255,87,74,0.1)' : 'rgba(38,193,154,0.1)',
                                                        marginLeft: 3,
                                                ...
                                                >
                                                    <Text style={[{ color: item.CheckResult == '未核实' ? '#666666' : item.CheckResult == '有异常' ? '#FF574A' : '#26C19A', textAlign: 'center', fontSize: 11 }]}>{item.CheckResult}</Text>
                                                </View>
                                                <View
                                                    style={{
                                                        alignItems: 'center',
                                                        justifyContent: 'center',
                                                        borderRadius: 5,
                                                        backgroundColor: item.Status == '待核实' ? 'rgba(47, 169, 255, 0.1)' : item.Status == '待复核' ? 'rgba(47, 169, 255, 0.1)' : 'rgba(38,193,154,0.1)',
                                                        marginLeft: 3,
                                                ...
                                                >
                                                    <Text style={[{ color: item.Status == '待核实' ? '#666666' : item.Status == '待复核' ? '#2FA9FF' : '#26C19A', textAlign: 'center', fontSize: 11 }]}>{item.Status}</Text>
                                                </View>
                                            </View>

...
export default class AlarmSectionList extends Component {
            ...
            <View key={index} style={{ width: SCREEN_WIDTH, minHeight: 45, paddingHorizontal: 10, paddingTop: 10 }}>
                <View style={{ minHeight: 45, width: SCREEN_WIDTH - 20, backgroundColor: 'white', paddingHorizontal: 10 }}>
                    <View style={{ marginTop: 15, width: SCREEN_WIDTH - 60, flexDirection: 'row', alignItems: 'center' }}>
                        ...
                        <Text numberOfLines={1} style={{ marginLeft: 5, fontSize: 14, color: '#333333' }}>{`${SentencedToEmpty(item, [0, 'EntName'], '--')}-${SentencedToEmpty(item, [0, 'PointName'], '--')}`}</Text>
                        ...
                        {item.map((typeItem, typeIndex) => {
                            return (
                                    ...
                                    onPress={() => {
                                        this.props.dispatch(
                                            NavigationActions.navigate({
                                                ...
                                                params: {
                                                    item: { ...typeItem, status: this.state.warningTypeCode }
                                                }
                                            })
                                        );
                                    }}
                                ...
                                >
                                    ...
                                    >
                                        <Text style={{ fontSize: 12, color: '#666666', maxWidth: (SCREEN_WIDTH - 150) / 2 }}>{SentencedToEmpty(typeItem, ['ModelName'], '--')}</Text>
    ...
    statusPageOnRefresh = () => {
        this.props.dispatch(createAction('alarmAnaly/updateState')({ ExceptionNumsByPointAuth: { status: -1 } }));
        this.props.dispatch(
    ...
    render() {
        const grouped = SentencedToEmpty(this.props, ['ExceptionNumsByPointAuth', 'data', 'Datas'], []).reduce((acc, obj) => {
            const key = obj.DGIMN;
            if (!acc[key]) {
                acc[key] = [];
            }

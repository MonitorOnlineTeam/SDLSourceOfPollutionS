...
export default class AlarmVerifyAnalyze extends PureComponent {
  ...
  renderPickedImage = () => {
    ...
    this.state.imagelist.map((item, key) => {
      ...
      // const source = item.attachID != '' ? { uri: `${ImageUrlPrefix}/${urlArray[urlArray.length - 1]}` } : require('../../../../images/addpic.png');
      const source =
        item.attachID != ''
          ? {
            uri: IMAGE_DEBUG
...
f021147d-e7c6-4c1d-9634-1d814ff9880a	多个污染物数据趋势一致 */
    ...
    if (
      ...
      WarningType == '178fd470-ca31-480a-835a-3322fd57a4f0'
    ) {
  ...
  render() {
    let {
      CheckedResult,
      IsRectificationRecord = '',
      CheckedDes,
      FileList,
    } = SentencedToEmpty(this.props.alarmVerifyDetail, ['data', 'Datas'], {
      CheckedResult: null,
      CheckedDes: null,
      FileList: [],
    });
    ...
    return (
      <StatusPage
        ...
        <ScrollView
              ...
              <TouchableOpacity
                onPress={() => {
                  ...
                  if (
                    ...
                    CheckedDes == null ||
                    (CheckedResult == '2' &&
                      (this.state.selectUntruthReason == '' ||
                        IsRectificationRecord == ''))
                  ) {
                    ShowToast('请完善相关信息');
                    return;
                    ...
                    this.props.dispatch(
                      createAction('alarmAnaly/commitVerify')({
                        params: {
                          ...
                          checkedResult: SentencedToEmpty(
                            this.props.alarmVerifyDetail,
                            ['data', 'Datas', 'CheckedResult'],
                            '',
                          ),
                          checkedDes: SentencedToEmpty(
                            this.props.alarmVerifyDetail,
                            ['data', 'Datas', 'CheckedDes'],
                            '',
                          ),
                          IsRectificationRecord: SentencedToEmpty(
                            this.props.alarmVerifyDetail,
                            ['data', 'Datas', 'IsRectificationRecord'],
                            '',
                          ),
                        ...
                        callback: () => {
                          if (this.alarmObj.length > 1) {
                            this.props.dispatch(
                              NavigationActions.back('AlarmAnalyList'),
                            );
                          } else {
          ...
          {this.props.commitVerifyResult.status == -1 ? (

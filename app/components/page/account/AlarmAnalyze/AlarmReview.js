...
export default class AlarmReview extends PureComponent {
    ...
    renderPickedImage = () => {
        ...
        this.state.imagelist.map((item, key) => {
            ...
            const source = item.attachID != '' ? { uri: IMAGE_DEBUG ? `${ImageUrlPrefix}/${urlArray[urlArray.length - 1]}` : `${ImageUrlPrefix}${ProxyCode}/${urlArray[urlArray.length - 1]}` } : require('../../../../images/addpic.png');
            console.log('source = ', source);
            rtnVal.push(
                <View View key={item.attachID} style={{ width: SCREEN_WIDTH / 4 - 5, height: SCREEN_WIDTH / 4 - 5 }}>
            ...
            this.setState(state => {
                ...
                img.map((item, index) => {
                    newImgUrls.push({
                        ...
                        url: `${ImageUrlPrefix}${ProxyCode}/${item.attachID}`,
                    });
                });

...
import { StyleSheet, Dimensions, View, Text, ListView, TouchableWithoutFeedback, TouchableNativeFeedback, TouchableOpacity, TouchableHighlight, Modal, ActivityIndicator } from 'react-native';
...
export default class ModalDropdown extends Component {
    ...
    _renderButton() {
        ...
        return (
            <TouchableOpacity style={{ width: 150, height: 30, alignItems: 'center', justifyContent: 'center' }} ref={button => (this._button = button)} disabled={disabled} accessible={accessible} onPress={this._onButtonPress}>
                {children || (
    ...
    _renderModal() {
        ...
        if (showDropdown && this._buttonFrame) {
            ...
            return (
                <Modal animationType={animationType} visible={true} transparent={true} onRequestClose={this._onRequestClose} supportedOrientations={['portrait', 'portrait-upside-down', 'landscape', 'landscape-left', 'landscape-right']}>
                    <TouchableWithoutFeedback accessible={accessible} disabled={!showDropdown} onPress={this._onModalPress}>
                        <View style={styles.modal}>
                            <View style={[styles.dropdown, dropdownStyle, frameStyle]}>{loading ? this._renderLoading() : this._renderDropdown()}</View>
                        </View>
    ...
    _renderRow = (rowData, sectionID, rowID, highlightRow) => {
        ...
        const row = !renderRow ? (
            <Text style={[styles.rowText, dropdownTextStyle, highlighted && styles.highlightedRowText, highlighted && dropdownTextHighlightStyle]}>{rowData[this.props.nameCode]}</Text>
        ) : (
    ...
    _onRowPress(rowData, sectionID, rowID, highlightRow) {
        ...
        if (!onSelect || onSelect(rowID, rowData) !== false) {
            ...
            const value = (renderButtonText && renderButtonText(rowData[this.props.nameCode])) || rowData[this.props.nameCode].toString();
            this._nextValue = value;

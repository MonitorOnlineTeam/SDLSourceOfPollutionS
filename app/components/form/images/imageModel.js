...
import { EncodeUtf8, createAction, NavigationActions, Storage, StackActions, ShowToast, SentencedToEmpty, createImageUrl, delay } from '../../../utils';
import * as authService from '../../../services/auth';
import { Model } from '../../../dvapack';
import { loadToken, saveRouterConfig, getToken, getEncryptData } from '../../../dvapack/storage';
...
export default Model.extend({
    ...
    effects: {
        ...
        *uploadimage({ payload: { image, images = [], callback, uuid } }, { call }) {
            ...
            if (images.length == 0) {
                // 单张
                _file = { uri: image.uri, type: 'multipart/form-data', name: 'image.jpg' };
                formdata.append('file', _file);
            } else if (images.length == 1) {
                // 单张
                _file = { uri: images[0].uri, type: 'multipart/form-data', name: 'image.jpg' };
                formdata.append('file', _file);
            } else {
                // 多张
                images.map((imageItem, index) => {
                    _file[index] = { uri: imageItem.uri, type: 'multipart/form-data', name: `image${index}.jpg` };
        ...
        //反馈上传图片-服务端加水印
        *uploadimageWaterMask({ payload: { image, images = [], callback, uuid, imageParams = {} } }, { call }) {
            let encryData = getEncryptData();
            ...
            if (images.length == 0) {
                ...
                var file = {};
                images.map((imageItem, index) => {
                    file[index] = { uri: imageItem.uri, type: 'multipart/form-data', name: `image${index}.jpg` };
                ...
                .then(res => {
                    if (res._bodyText && res._bodyText != '') {
                        ...
                        if (obj.IsSuccess) {
                            ...
                            callback('上传失败:' + SentencedToEmpty(obj, ['Message'], '无错误信息'), false);
                        }
                    } else {
                        callback('上传失败', false);
                    }

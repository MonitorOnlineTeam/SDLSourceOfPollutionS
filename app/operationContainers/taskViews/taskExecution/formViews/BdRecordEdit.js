...
import { KeyboardAvoidingView, ScrollView, View, Text, StyleSheet, Platform, Image, TouchableOpacity, Dimensions } from 'react-native';
import { NavigationActions, createAction, ShowToast, SentencedToEmpty } from '../../../../utils';
import { connect } from 'react-redux';
import moment from 'moment';

import globalcolor from '../../../../config/globalcolor';
import { StatusPage, SimpleLoadingComponent, SimplePickerSingleTime, AlertDialog } from '../../../../components';
...
@connect(
    ({ bdRecordModel }) => ({
        ...
        delSubtableStatus: bdRecordModel.delSubtableStatus
    }),
    null,
    null,
    { withRef: true }
...
class BdRecordEdit extends PureComponent {
    static navigationOptions = ({ navigation }) => {
        return {
            ...
            headerRight:
                SentencedToEmpty(navigation, ['state', 'params', 'ItemData', 'ID'], '') != '' ? (
                    <TouchableOpacity
                        onPress={() => {
    ...
    render() {
        ...
        return (
            <StatusPage style={styles.container} status={200}>
                <KeyboardAvoidingView style={{ alignItems: 'center' }} behavior={Platform.OS == 'ios' ? 'padding' : ''} keyboardVerticalOffset={100}>
                    <ScrollView style={styles.scroll}>
                        <View style={styles.scrollContent}>
                            ...
                            <View style={[styles.item, { backgroundColor: '#ffffff', flexDirection: 'column' }]}>
                                <FormText
                                    last={true}
                                    label="评价结果"
                                    itemHeight={40}
                            ...
                            >
                                ...
                                <Image style={{ width: 10, height: 10, marginLeft: 8, marginRight: 12 }} source={ic_arrows_down} />
                            </TouchableOpacity>
                            <View>
                            ...
                            >
                                <Text style={styles.line} />
                                <Text style={{ fontSize: 15, color: '#0080ff', width: '100%', textAlign: 'center', lineHeight: 40 }}>+ 添加测量记录</Text>
                            </TouchableOpacity>
                        </View>
                    </ScrollView>
                ...
                {this.props.delSubtableStatus.status == -2 ? <SimpleLoadingComponent message={'删除中'} /> : null}
    ...
    getTimeSelectOption = index => {
        ...
        return {
            ...
            type: type,
            defaultTime: this.state.TestResult[index].TestTime || moment().format('YYYY-MM-DD HH:mm:ss'),
    ...
    getEndTimeSelectOption = index => {
        ...
        return {
            formatStr: 'YYYY-MM-DD HH:mm',
            type: type,
            defaultTime: this.state.TestResult[index].EndTime || moment().format('YYYY-MM-DD HH:mm:ss'),
            onSureClickListener: time => {
                let TestResult = this.state.TestResult;
        ...
        return (
            <View key={`item${i}`} style={styles.itemcontainer}>
                <View style={styles.itemtitle}>
                    <Text style={{ color: '#666666', fontSize: 12 }}>{'监测记录(' + i + ')'}</Text>
                    <TouchableOpacity
                        onPress={() => {
                            this.del(index);
                        }}
                    >
                        <Text style={{ color: '#666666', fontSize: 12, width: 48, lineHeight: 37, textAlign: 'right' }}>删除</Text>
                    </TouchableOpacity>
                </View>
                <View style={styles.itemContent}>
                    <View style={styles.item}>
                        <Text style={styles.textlabel}>监测开始时间：</Text>
                        <SimplePickerSingleTime style={{ width: 160 }} option={this.getTimeSelectOption(index)} />
                    </View>
                    <Text style={styles.line} />
                    <View style={styles.item}>
                        <Text style={styles.textlabel}>监测结束时间：</Text>
                        <SimplePickerSingleTime style={{ width: 160 }} option={this.getEndTimeSelectOption(index)} />
                    </View>
                    <Text style={styles.line} />
    ...
    commit = () => {
        ...
        CemsTextValue = ((CemsTextValue * 1.0) / this.state.TestResult.length).toFixed(3);
        // 修改本地的数据
        this.props.dispatch(
            createAction('bdRecordModel/saveItem')({
                params: {
                    ItemID: this.state.ItemID,
                    ItemName: this.state.Name,
        ...
        // 提交本地的数据
        this.props.dispatch(createAction('bdRecordModel/saveForm')({ params: {} }));
    };

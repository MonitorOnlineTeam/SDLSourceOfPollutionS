...
class ConsumableReplace extends Component {
  ...
  jumpToForm = params => {
    ...
    this.props.navigation.navigate('ConsumableReplaceForm', {
      params: {
        TaskID: TaskID,
        TypeID: TypeID,
  ...
  onOK = (record, resolve) => {
    ...
    this.props.dispatch(
      createAction('patrolModel/AddOrUpdateRecord')({
        actionType: 'AddOrUpdateConsumablesReplaceRecordZB',
        params: body,
        callback: result => {
  ...
  onSubmit = () => {
    ...
    this.props.dispatch(
      createAction('patrolModel/AddOrUpdateRecord')({
        actionType: 'SubmitConsumablesReplaceRecordZB',
        params: body,
        callback: result => {
          if (result.status == 200) {
            this.props.dispatch(
  ...
  onDeleteRecord = () => {
    ...
    this.props.dispatch(
      createAction('patrolModel/DeleteRecord')({
        actionType: 'DeletetConsumablesReplaceRecordZB',
        ...
        <View style={styles.standardItem}>
          <View style={styles.standardContent}>
            <View style={styles.standardRow}>
              <Text style={styles.label}>易耗品名称：</Text>
              <Text style={styles.value}>{record.ConsumablesName}</Text>
            </View>
            <View style={styles.standardRow}>
              <View style={{...styles.standardRowItem, flex: 2}}>
                <Text style={styles.label}>说明书建议周期：</Text>
                <Text style={styles.value}>{record.SuggestionDate}</Text>
              </View>
              <View
            ...
            <View style={styles.standardRow}>
              <Text style={styles.label}>上次更换日期：</Text>
              <Text style={styles.value}>
                {moment(record.LastDate).format('YYYY-MM-DD')}
              </Text>
            </View>
            <View style={styles.standardRow}>
              <Text style={styles.label}>本次更换日期：</Text>
              <Text style={styles.value}>

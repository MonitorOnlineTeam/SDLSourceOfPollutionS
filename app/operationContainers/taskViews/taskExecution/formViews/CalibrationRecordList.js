...
import { NavigationActions, createAction, createNavigationOptions, SentencedToEmpty, StackActions, ShowToast } from '../../../../utils';
// import { getToken, } from '../../dvapack/storage';
import { StatusPage, Touchable, PickerTouchable, SimpleLoadingComponent, MoreSelectTouchable, SimpleLoadingView } from '../../../../components';
                        ...
                        >
                            ...
                            <View style={[{ width: SCREEN_WIDTH - 24, flexDirection: 'row', marginTop: 10 }]}>
                                <View style={[{ flexDirection: 'row', height: 30, flex: 1 }]}>
                                    ...
                                    <Text numberOfLines={1} style={[{ width: (SCREEN_WIDTH - 24) / 2 - 100, fontSize: 14, color: globalcolor.datepickerGreyText }]}>{item.LdLastCalibrationValue && item.LdLastCalibrationValue != '' ? item.LdLastCalibrationValue : '--'}</Text>
                                </View>
                                <View style={[{ flexDirection: 'row', height: 30, flex: 1 }]}>
                                    <Text style={[{ fontSize: 14, color: globalcolor.datepickerGreyText }]}>{'零点漂移：'}</Text>
                                    <Text numberOfLines={1} style={[{ width: (SCREEN_WIDTH - 24) / 2 - 80, fontSize: 14, color: globalcolor.datepickerGreyText }]}>{item.LdPy && item.LdPy != '' ? item.LdPy : '--'}</Text>
                                </View>
                            </View>
                            <View style={[{ width: SCREEN_WIDTH - 24, flexDirection: 'row', marginTop: 10 }]}>
                                <View style={[{ flexDirection: 'row', height: 30, flex: 1 }]}>
                                    <Text style={[{ fontSize: 14, color: globalcolor.datepickerGreyText }]}>{'校前测试：'}</Text>
                                    <Text numberOfLines={1} style={[{ width: (SCREEN_WIDTH - 24) / 2 - 80, fontSize: 14, color: globalcolor.datepickerGreyText }]}>{item.LdCalibrationPreValue && item.LdCalibrationPreValue != '' ? item.LdCalibrationPreValue : '--'}</Text>
                                </View>
                                <View style={[{ flexDirection: 'row', height: 30, flex: 1 }]}>
                                ...
                                item.ItemId == 543 ? null
                                    : <View>
                                        <View style={[{ flexDirection: 'row', height: 30, marginTop: 10, alignItems: 'center' }]}>
                                            ...
                                            <Text style={[{ fontSize: 14, color: globalcolor.taskImfoLabel }]}>{`(${item.FxyLc ? item.FxyLc : ' '})`}</Text>
                                        </View>
                                        <View style={[{ width: SCREEN_WIDTH - 24, flexDirection: 'row', marginTop: 10 }]}>
                                            <View style={[{ flexDirection: 'row', height: 30, flex: 1 }]}>
                                                <Text style={[{ fontSize: 14, color: globalcolor.datepickerGreyText }]}>{'上次校准后：'}</Text>
                                                <Text numberOfLines={1} style={[{ width: (SCREEN_WIDTH - 24) / 2 - 100, fontSize: 14, color: globalcolor.datepickerGreyText }]}>{item.LcLastCalibrationValue && item.LcLastCalibrationValue != '' ? item.LcLastCalibrationValue : '--'}</Text>
                                            </View>
                                            <View style={[{ flexDirection: 'row', height: 30, flex: 1 }]}>
                                                <Text style={[{ fontSize: 14, color: globalcolor.datepickerGreyText }]}>{'量程漂移：'}</Text>
                                                <Text numberOfLines={1} style={[{ width: (SCREEN_WIDTH - 24) / 2 - 80, fontSize: 14, color: globalcolor.datepickerGreyText }]}>{item.LcPy && item.LcPy != '' ? item.LcPy : '--'}</Text>
                                            </View>
                                        </View>
                                        <View style={[{ width: SCREEN_WIDTH - 24, flexDirection: 'row', marginTop: 10 }]}>
                                            <View style={[{ flexDirection: 'row', height: 30, flex: 1 }]}>
                                                <Text style={[{ fontSize: 14, color: globalcolor.datepickerGreyText }]}>{'校前测试：'}</Text>
                                                <Text numberOfLines={1} style={[{ width: (SCREEN_WIDTH - 24) / 2 - 80, fontSize: 14, color: globalcolor.datepickerGreyText }]}>{item.LcCalibrationPreValue && item.LcCalibrationPreValue != '' ? item.LcCalibrationPreValue : '--'}</Text>
                                            </View>
                                            <View style={[{ flexDirection: 'row', height: 30, flex: 1 }]}>
                                                <Text style={[{ fontSize: 14, color: globalcolor.datepickerGreyText }]}>{'是否正常：'}</Text>
            ...
            buttons: [
                ...
                },
                {
                    ...
                    onpress: () => {
                        ...
                        this.props.dispatch(createAction('calibrationRecord/deleteJzItem')({ params: { ID: idStr } }));
                    }
                }
            ]
        };
            ...
            >
                <View style={styles.container}>
                    <TouchableOpacity
                        ...
                        onPress={() => {
                            this.props.dispatch(NavigationActions.navigate({ routeName: 'CalibrationRecordTime', params: {} }));
                        }}
                    >
                        <View style={[{ width: SCREEN_WIDTH, paddingHorizontal: 24, backgroundColor: globalcolor.white }]}>
                            <View style={[{ flexDirection: 'row', height: 45, marginTop: 10, borderBottomWidth: 1, alignItems: 'center', borderBottomColor: globalcolor.borderBottomColor }]}>
                                <Text style={[{ fontSize: 15, color: globalcolor.taskImfoLabel }]}>校准开始时间：</Text>
                                <View style={[{ flex: 1, flexDirection: 'row', height: 45, alignItems: 'center' }]}>
                                    <Text style={[{ fontSize: 15, color: globalcolor.taskFormLabel, flex: 1, textAlign: 'right' }]}>{SentencedToEmpty(this.props.MainInfo, ['AdjustStartTime'], '----/--/-- --:--')}</Text>
                                </View>
                            </View>
                            <View style={[{ flexDirection: 'row', height: 45, marginTop: 10, borderBottomWidth: 1, alignItems: 'center', borderBottomColor: globalcolor.borderBottomColor, marginBottom: 10 }]}>
                                <Text style={[{ fontSize: 15, color: globalcolor.taskImfoLabel }]}>校准结束时间：</Text>
                                <View style={[{ flex: 1, flexDirection: 'row', height: 45, alignItems: 'center' }]}>
                                    <Text style={[{ fontSize: 15, color: globalcolor.taskFormLabel, flex: 1, textAlign: 'right' }]}>{SentencedToEmpty(this.props.MainInfo, ['AdjustEndTime'], '----/--/-- --:--')}</Text>
                                </View>
                            </View>
                        </View>
                    </TouchableOpacity>
                    {SentencedToEmpty(this.props, ['MainInfo', 'AdjustEndTime'], '') == '' || SentencedToEmpty(this.props, ['MainInfo', 'AdjustStartTime'], '') == '' ? (
                        <TouchableOpacity
                            onPress={() => {
                    ...
                    {this.props.editstatus.status == -1 ? <SimpleLoadingView message={'删除中'} /> : null}
                    {this.props.jzDeleteResult.status == -1 ? <SimpleLoadingView message={'删除中'} /> : null}
                    <AlertDialog options={delConfigItemsAlert} ref="delConfigItemsAlert" />
                </View>
            </StatusPage>
        );
    }

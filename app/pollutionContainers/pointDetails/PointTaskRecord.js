    ...
    this.props.dispatch(
      createAction('enterpriseListModel/getOperationHistoryRecordByDGIMN')({
        params: {
          ...
          DGIMN: SentencedToEmpty(
            this.props,
            ['route', 'params', 'params', 'DGIMN'],
            // ['navigation', 'state', 'params', 'DGIMN'],
            '',
          ),
          pageIndex: 1,
          pageSize: 20,
  ...
  getRangeDaySelectOption = () => {
    ...
    return {
      ...
      onSureClickListener: (start, end) => {
        ...
        this.props.dispatch(
          createAction('enterpriseListModel/updateState')({
            TaskRecordbeginTime: startMoment.format('YYYY-MM-DD 00:00:00'),
            TaskRecordendTime: endMoment.format('YYYY-MM-DD 23:59:59'),
          }),
        );
        // this.list.onRefresh();
      ...
      <View style={styles.container}>
        ...
        <StatusPage
          ...
          <FlatListWithHeaderAndFooter
            ...
            hasMore={() => {
              return (
                this.props.TaskRecordData.length < this.props.TaskRecordTotal
              );
            }}
            onRefresh={index => {
            ...
            onEndReached={index => {
              ...
              this.props.dispatch(
                createAction(
                  'enterpriseListModel/getOperationHistoryRecordByDGIMN',
                )({
                  params: {
                    IsQueryAllUser: true,
                    exceptionType: '0',
                  ...
                  <View
                    ...
                    <SDLText
                      style={{
                        ...
                        color: '#333',
                        lineHeight: 18,
                      }}>{`${item.EnterpriseName}-${item.PointName}`}</SDLText>
                    <View
                      style={[
                        {
                          flexDirection: 'row',
                      ...
                      <View style={[{flex: 1}]}>
                        <View style={{flexDirection: 'row'}}>
                          <View
                            ...
                            <SDLText style={{color: '#666', marginRight: 8}}>
                              {item.TaskFromText}
                            </SDLText>
                            {SentencedToEmpty(
                              item,
                              ...
                              return (
                                <TextLabel
                                  key={`tag_${index}_${key}`}
                                  style={{marginLeft: key == 0 ? 0 : 3}}
                                  color={'#75A5FB'}
                                  text={tag.TypeName}
                                />
                              );
                              ...
                              .map((et, key) => {
                                return (
                                  <View
                                    ...
                                    {key > 0 ? (
                                      <SDLText style={{color: '#666'}}>
                                        {' | '}
                                      </SDLText>
                                    ) : null}
                                    <SDLText style={{color: '#D28B38'}}>
                                      {et}
                                    </SDLText>
                                  </View>
                                );
                        ...
                        <SDLText
                          style={{marginTop: 10, flex: 1, color: '#666'}}>
                          {'时间：' +
                            (SentencedToEmpty(item, ['CompleteTime'], '') == ''
                              ? '---/--/-- --:--'
                              : moment(item.CompleteTime).format(
                                  'YYYY/MM/DD HH:mm',
                                ))}
                        </SDLText>
                      </View>
                      {SentencedToEmpty(item, ['TaskStatus'], -1) == 3 ? (
                        <Image
                          style={{width: 100, height: 100, marginRight: 13}}
                          source={require('../../images/img_record_complete.png')}
                        />
                      ) : SentencedToEmpty(item, ['TaskStatus'], -1) == 10 ? (
                        <Image
                          style={{width: 100, height: 100, marginRight: 13}}
                          source={require('../../images/img_record_closed.png')}
                        />
                      ) : (
                        <View
                          style={{width: 100, height: 100, marginRight: 13}}

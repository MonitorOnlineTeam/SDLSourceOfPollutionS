...
import { createNavigationOptions, SentencedToEmpty, createAction, NavigationActions, ShowToast } from '../../utils';
import { StatusPage, FlatListWithHeaderAndFooter, Touchable, SDLText, OperationAlertDialog, AlertDialog, DeclareModule } from '../../components';
...
export default class AbnormalityReportLst extends Component {
    ...
    onRefresh = index => {
        this.props.dispatch(createAction('pointDetails/updateState')({ abnormalListIndex: index, abnormalListTotal: 0 }));
        /**
         */
        this.props.dispatch(createAction('pointDetails/getAbnormalList')({ setListData: this.list.setListData }));
    };

    statusPageOnRefresh = () => {
        this.props.dispatch(createAction('pointDetails/updateState')({ abnormalListIndex: 1, abnormalListTotal: 0, abnormalListResult: { status: -1 } }));
    ...
    showSpinner = () => {
        this.setState({
            isVisible: true,
            spinnerRect: { x: SCREEN_WIDTH - 85, y: -SCREEN_HEIGHT / 2 + 70, width: 100, height: 300 }
        });
    };
    ...
    onItemClick(spinnerItem) {
        ...
        if (spinnerItem == '上报') {
            if (CURRENT_PROJECT == POLLUTION_ORERATION_PROJECT) {
                this.props.dispatch(NavigationActions.navigate({ routeName: 'SuspendReport', params: {} }));
            } else {
                ...
                >
                    <View style={styles.container}>
                        <FlatListWithHeaderAndFooter
                            ...
                            pageSize={20}
                            hasMore={() => {
                                return this.props.abnormalList.length < this.props.abnormalListTotal;
                            }}
                            onRefresh={index => {
                                this.onRefresh(index);
                            }}
                            onEndReached={index => {
                                this.props.dispatch(createAction('pointDetails/updateState')({ abnormalListIndex: index }));
                                this.props.dispatch(createAction('pointDetails/getAbnormalList')({ setListData: this.list.setListData }));
                            }}
                                        ...
                                        <View style={[{ width: SCREEN_WIDTH, alignItems: 'center', marginTop: 0 }]}>
                                            <View style={[{ width: SCREEN_WIDTH, borderColor: globalcolor.borderLightGreyColor, backgroundColor: globalcolor.white, paddingLeft: 13, paddingRight: 13 }]}>
                                                <View style={[{ flexDirection: 'row', height: 30, marginTop: 10 }]}>
                                                    <Text style={[{ fontSize: 14, color: globalcolor.taskImfoLabel }]}>开始时间：</Text>
                                                    <Text style={[{ fontSize: 14, color: globalcolor.taskImfoLabel }]}>{SentencedToEmpty(item, ['BeginTime'], '未填写')}</Text>
                                                </View>
                                                <View style={[{ flexDirection: 'row', height: 30 }]}>
                                                    <Text style={[{ fontSize: 14, color: globalcolor.taskImfoLabel }]}>截止时间：</Text>
                                                    <Text style={[{ fontSize: 14, color: globalcolor.taskImfoLabel }]}>{SentencedToEmpty(item, ['EndTime'], '未填写')}</Text>
                                                </View>
                                                <View style={[{ flexDirection: 'row', height: 30 }]}>
                                                    <Text style={[{ fontSize: 14, color: globalcolor.taskImfoLabel }]}>异常描述：</Text>
                                                    <Text style={[{ fontSize: 14, color: globalcolor.taskImfoLabel }]}>{SentencedToEmpty(item, ['Msg'], '---')}</Text>
                                                </View>
                                                ...
                                                <View style={[{ flexDirection: 'row', height: 50, justifyContent: 'space-between', alignItems: 'center' }]}>
                                                    <TouchableOpacity
                                                        onPress={() => {
                                                            this.props.dispatch(NavigationActions.navigate({ routeName: 'AbnormalityReportDetails', params: { item } }));
                                                        }}
                                                    ...
                                                    <TouchableOpacity
                                                        onPress={() => {
                                                            this.props.dispatch(NavigationActions.navigate({ routeName: 'AbnormalityReport', params: { item } }));
                                                        }}
                                                        style={{ alignItems: 'center', justifyContent: 'center', flex: 1 }}
                                                    >
                    ...
                    <Touchable
                        onPress={() => {
                            if (CURRENT_PROJECT == POLLUTION_ORERATION_PROJECT) {
                                this.props.dispatch(NavigationActions.navigate({ routeName: 'AbnormalityReport' }));
                            } else {
                                ShowToast('非运维人员无法进行添加');
                            }
                        }}

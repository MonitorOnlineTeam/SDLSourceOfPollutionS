...
export default class OperationLog extends PureComponent {
  ...
  onRefresh = index => {
    this.props.dispatch(
      createAction('pointDetails/updateState')({
        operationLogsIndex: index,
        operationLogsTotal: 0,
      }),
    );
    this.props.dispatch(
      createAction('pointDetails/getOperationLogs')({
        setListData: this.list.setListData,
        setDataArr: this.setDataArr,
      }),
    );
  };
  ...
  getItemHeight = (index, outerItem) => {
    if (
      SentencedToEmpty(outerItem, ['TypeID'], 0) == 0 ||
      SentencedToEmpty(outerItem, ['MainFormID']) == ''
    ) {
      return 220;
    } else {
      let topBorder = 20,
            ...
            if (outerItem.Type == '0') {
              ...
              this.props.dispatch(
                StackActions.push({
                  ...
                  params: {
                    ...outerItem,
                    createForm: outerItem.FormMainID != null ? true : false,
                    isEdit: 'noEdit',
                    viewTitle: SentencedToEmpty(
                      outerItem,
                      ['Abbreviation'],
                      '图片表单',
                    ),
              ...
              <View style={[{flexDirection: 'row', alignItems: 'center'}]}>
                ...
                <SDLText
                  ...
                  style={[
                    {
                      color: '#4aa2ff',
                      backgroundColor: transformColorToTransparency('#4aa2ff'),
                      paddingHorizontal: 8,
                      marginLeft: 10,
                      lineHeight: 20,
                    },
                  ...
                  {this.getTypeAbbreviation(
                    SentencedToEmpty(outerItem, ['TypeID'], -1),
                  )}
                </SDLText>
              </View>
              <SDLText
        ...
        <StatusPage
          ...
          <View style={styles.container}>
            <FlatListWithHeaderAndFooter
              ...
              hasMore={() => {
                return (
                  this.props.operationLogsListData.length <
                  this.props.alarmRecordsTotal
                );
              }}
              onRefresh={index => {
                    ...
                    <View
                      ...
                      <View
                        style={[
                          {
                            width: 14,
                            height: this.getItemHeight(index, item),
                            marginLeft: 18,
                            alignItems: 'center',
                          },
                        ]}>
                          ...
                          style={[
                            {
                              flex: 1,
                              width: 1,
                              backgroundColor: index == 0 ? '#f2f2f2' : '#999',
                            },
                          ]}
                        />
                        <View

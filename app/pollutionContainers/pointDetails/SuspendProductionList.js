...
export default class SuspendProductionList extends Component {
    ...
    onRefresh = index => {
        this.props.dispatch(createAction('pointDetails/updateState')({ outputStopListIndex: index, outputStopListTotal: 0 }));
        /**
         */
        this.props.dispatch(createAction('pointDetails/getOutputStopList')({ setListData: this.list.setListData }));
    };

    statusPageOnRefresh = () => {
        this.props.dispatch(createAction('pointDetails/updateState')({ outputStopListIndex: 1, outputStopListTotal: 0, outputStopListResult: { status: -1 } }));
                ...
                >
                    <View style={styles.container}>
                        <FlatListWithHeaderAndFooter
                            ...
                            pageSize={20}
                            hasMore={() => {
                                return this.props.outputStopList.length < this.props.outputStopListTotal;
                            }}
                            onRefresh={index => {
                                this.onRefresh(index);
                            }}
                            onEndReached={index => {
                                this.props.dispatch(createAction('pointDetails/updateState')({ outputStopListIndex: index }));
                                this.props.dispatch(createAction('pointDetails/getOutputStopList')({ setListData: this.list.setListData }));
                            }}
                                    ...
                                    <Touchable>
                                        <View style={[{ width: SCREEN_WIDTH, alignItems: 'center', marginTop: 0 }]}>
                                            <View style={[{ width: SCREEN_WIDTH, borderColor: globalcolor.borderLightGreyColor, backgroundColor: globalcolor.white, paddingLeft: 13, paddingRight: 13 }]}>
                                                ...
                                                <View style={[{ flexDirection: 'row', height: 30 }]}>
                                                    <Text style={[{ fontSize: 14, color: globalcolor.taskImfoLabel }]}>原因备注：</Text>
                                                    <Text style={[{ fontSize: 14, color: globalcolor.taskImfoLabel }]}>{SentencedToEmpty(item, ['StopDescription'], '---')}</Text>
                                                </View>
                                                ...
                                                <View style={[{ flexDirection: 'row', height: 50, justifyContent: 'space-between', alignItems: 'center' }]}>
                                                    <TouchableOpacity
                                                        onPress={() => {
                                                            this.props.dispatch(NavigationActions.navigate({ routeName: 'SuspendProductionDetail', params: { item } }));
                                                        }}
                                                    ...
                                                    <TouchableOpacity
                                                        onPress={() => {
                                                            if (CURRENT_PROJECT == POLLUTION_ORERATION_PROJECT) {
                                                                this.props.dispatch(NavigationActions.navigate({ routeName: 'SuspendReport', params: { item } }));
                                                            } else {

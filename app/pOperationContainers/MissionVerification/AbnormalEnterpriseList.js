...
import { Text, TouchableOpacity, View, Image, Platform, DeviceEventEmitter } from 'react-native';
import { connect } from 'react-redux';
import { StatusPage, FlatListWithHeaderAndFooter, SDLText, SimplePickerRangeDay, SimplePicker } from '../../components';
...
export default class AbnormalEnterpriseList extends Component {
    ...
    componentDidMount() {
        this.subscription = DeviceEventEmitter.addListener('refreshSec', this.statusPageOnRefresh);
    ...
    renderItem = ({ item, index }) => {
        return (
            <View key={index} style={{ width: SCREEN_WIDTH, minHeight: 45, paddingHorizontal: 10, paddingTop: 10 }}>
                <View style={{ minHeight: 45, width: SCREEN_WIDTH - 20, backgroundColor: 'white', paddingHorizontal: 10 }}>
                    <View style={{ marginTop: 15, width: SCREEN_WIDTH - 60, flexDirection: 'row', alignItems: 'center' }}>
                        ...
                        <Text numberOfLines={1} style={{ marginLeft: 5, fontSize: 14, color: '#333333' }}>{`${SentencedToEmpty(item, ['EntName'], '--')}-${SentencedToEmpty(item, ['PointName'], '--')}`}</Text>
                    </View>
                    <View style={{ width: SCREEN_WIDTH - 40, minHeight: 48, flexDirection: 'row', flexWrap: 'wrap', marginTop: 15 }}>
                        ...
                        {item.WarningDatas.map((typeItem, typeIndex) => {
                            return (
                                ...
                                >
                                    ...
                                    >
                                        <Text style={{ fontSize: 12, color: '#666666', maxWidth: (SCREEN_WIDTH - 150) / 2 }}>{SentencedToEmpty(typeItem, ['WarningName'], '--')}</Text>
                                        <Text style={{ fontSize: 12, color: '#4AA0FF' }}>{`${SentencedToEmpty(typeItem, ['WarningCount'], '0')}个`}</Text>
                                    </View>
                                </TouchableOpacity>
                            );
                        })
    ...
    statusPageOnRefresh = () => {
        ...
        this.props.dispatch(createAction('abnormalTask/GetClueDatas')({ setListData: this.list ? this.list.setListData : null }));
    };
    ...
    render() {
        const grouped = SentencedToEmpty(this.props, ['ExceptionNumsByPointAuth', 'data', 'Datas', 'showWarnings'], []).reduce((acc, obj) => {
            const key = obj.DGIMN;
            if (!acc[key]) {
                acc[key] = [];
            }
        ...
        return (
            <View style={{ width: SCREEN_WIDTH, flex: 1 }}>
                <View style={[{ width: SCREEN_WIDTH, height: 45, flexDirection: 'row', justifyContent: 'center', backgroundColor: '#fff', marginBottom: 10, paddingLeft: 20, paddingRight: 20 }]}>
                    <SimplePickerRangeDay option={this.getRangeDaySelectOption()} />
                </View>

                <StatusPage
                    status={SentencedToEmpty(this.props, ['ExceptionNumsByPointAuth', 'status'], 200)}
                    //页面是否有回调按钮，如果不传，没有按钮，
                    emptyBtnText={'重新请求'}
                    errorBtnText={'点击重试'}
                    onEmptyPress={() => {
                ...
                >
                    <FlatListWithHeaderAndFooter
                        ...
                        hasMore={() => {
                            return this.props.clueDatasList.length < this.props.clueDatasTotal;
                        }}
                        onRefresh={index => {

 ...
 * @LastEditTime: 2023-10-24 14:09:24
 * @FilePath: /SDLMainProject36/app/pOperationContainers/tabView/chengTaoXiaoXi/ChengTaoTaskRecord.js
 */
 ...
 */
@connect(({ CTModel }) => ({
    completedPageSize: CTModel.completedPageSize,
    serviceDispatchCompletedData: CTModel.serviceDispatchCompletedData,
    serviceDispatchCompletedResult: CTModel.serviceDispatchCompletedResult
}))
export default class ChengTaoTaskRecord extends Component {
    ...
    statusPageOnRefresh = () => {
        ...
        this.props.dispatch(createAction('CTModel/getServiceDispatchCompleted')({}));
    }
    ...
    onRefresh = () => {
        ...
        this.props.dispatch(createAction('CTModel/getServiceDispatchCompleted')({ setListData: this.list.setListData }));
    }
    ...
    render() {
        return (
            <StatusPage
                backRef={true}
                status={SentencedToEmpty(this.props.serviceDispatchCompletedResult
                    , ['status'], 200)}
                //页面是否有回调按钮，如果不传，没有按钮，
                emptyBtnText={'重新请求'}
                errorBtnText={'点击重试'}
            ...
            >
                <View style={[{ width: SCREEN_WIDTH, flex: 1 }]}>
                    <FlatListWithHeaderAndFooter
                        ...
                        pageSize={this.props.completedPageSize}
                        ItemSeparatorComponent={() => <View style={[{ width: SCREEN_WIDTH, height: 1, backgroundColor: '#f2f2f2' }]} />}
                        hasMore={() => {
                            return SentencedToEmpty(this.props, ['serviceDispatchCompletedResult', 'data', 'Total'], 0)
                                > SentencedToEmpty(this.props, ['serviceDispatchCompletedData'], []).length
                        }}
                        onRefresh={index => {
                        ...
                        onEndReached={index => {
                            this.props.dispatch(createAction('CTModel/updateState')({
                                completedPageIndex: index
                            }));
                            this.props.dispatch(createAction('CTModel/getServiceDispatchCompleted')({ setListData: this.list.setListData }));
                        }}
                        renderItem={this._renderItem}
                        data={this.props.serviceDispatchCompletedData}
                    />
                </View>
            </StatusPage>
        )

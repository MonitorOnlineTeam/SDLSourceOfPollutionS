 ...
 * @LastEditTime: 2024-11-25 14:29:25
...
@connect(({ CTModel, CTSparePartsChangeModel }) => ({
    ...
    upDateEquipmentInstalled: CTSparePartsChangeModel.upDateEquipmentInstalled, // 点位情况
    failureCause: CTSparePartsChangeModel.failureCause, // 故障原因列表
}))
export default class SparePartsChangeUpdate extends Component {
    ...
    constructor(props) {
        ...
        console.log('naviParams = ', naviParams);
        naviParams.failureCause = naviParams.FailureCause;
        this.state = {
            isVisible: false,
            spinnerRect: {},
            data: { Status: 1 },
    ...
    componentDidMount() {
        ...
        this.props.dispatch(createAction('CTModel/getProjectEntPointSysModel')({
            ...
            , callback: (result) => {
                ...
                const incomeData = SentencedToEmpty(naviParams, ['cList', 0], {});
                let stateParams = {
                    projectList: projectList
                };
                let indexe = projectList.findIndex((item) => {
                    return item.ProjectId == incomeData.projectId;
                });
                const entList = projectList[indexe].EntList;
                if (indexe != -1) {
                    stateParams.entList = entList;
                }
                let indexp = entList.findIndex((item) => {
                    return item.EntId == incomeData.entId;
                });
                const pointList = entList[indexp].PointList;
                if (indexp != -1) {
                    stateParams.pointList = pointList;
                }
                if (this.props.upDateEquipmentInstalled == 1) {
                    let indexm = pointList.findIndex((item) => {
                        return item.PointId == incomeData.pointId;
                    });
                    const systemModleList = SentencedToEmpty(pointList, [indexm, 'systemModleList'], []);
                    if (indexm != -1) {
                        stateParams.systemModleList = systemModleList;
                    }
                }
                this.setState(
                    stateParams
    ...
    checkParams = () => {
        ...
        const comiteParamsData = SentencedToEmpty(this.state
            , ['comiteParams', 'cList', 0], {});
        if (this.props.upDateEquipmentInstalled != 1
            && this.props.upDateEquipmentInstalled != 2
        ) {
                ...
                >
                    <ItemOutLayout >
                        <Form2Switch
                            ...
                            value={this.props.upDateEquipmentInstalled}
                            onChange={(value) => {
                                this.props.dispatch(createAction('CTSparePartsChangeModel/updateState')({
                                    upDateEquipmentInstalled: value
                                }));
                                // 如果选中2，则清掉点位选项
                                if (value == 1) {
                                    this.upDateComiteData([
                    ...
                    />
                    {this.props.upDateEquipmentInstalled == 1 ?
                        <CurrentPicker
                            label={'点位名称'}
                            codeKey={'PointId'}
                            nameKey={'PointName'}
                        ...
                        this.props.upDateEquipmentInstalled == 1 ?
                            ...
                            : this.props.upDateEquipmentInstalled == 2 ?
                                ...
                                >
                                    ...
                                    >
                                        <Text style={[{ color: 'red', marginRight: 4 }]}>{'*'}</Text>
                                        <Text style={[{ fontSize: 14, color: '#333333' }]}>{`系统型号`}</Text>
                                        <TouchableOpacity
                                            style={[{
                                                flex: 1, height: 44
                                                , flexDirection: 'row'

 ...
 * @Description: 
 * @LastEditors: hxf
 * @Date: 2024-04-25 10:16:33
 * @LastEditTime: 2024-06-07 15:07:38
 * @FilePath: /SDLMainProject37/app/pOperationContainers/tabView/chengTaoXiaoXi/ServiceReportRectification/ServiceReportRectificationList.js
 */
...
@connect(({ CTServiceReportRectificationModel }) => ({
    tabSelectedIndex: CTServiceReportRectificationModel.tabSelectedIndex,
    serviceStatusNumResult: CTServiceReportRectificationModel.serviceStatusNumResult
}))
export default class ServiceReportRectificationList extends Component {
    ...
    componentDidMount() {
        ...
        this.props.dispatch(createAction('CTServiceReportRectificationModel/updateState')({
            serviceStatusNumResult: { status: -1 }
        }));
                    ...
                    <View style={[{ flex: 1, height: 44, flexDirection: 'row' }]}>
                        <TouchableOpacity style={[{ flex: 1, height: 44 }]}
                            onPress={() => {
                                ...
                                this.props.dispatch(
                                    createAction(
                                        'CTServiceReportRectificationModel/getStayCheckServices'
                                    )({}));
                            }}
                        >
                            <View
                                ...
                                <View style={[{
                                    ...
                                    <Text
                                        ...
                                        style={[{
                                            ...
                                            , ['serviceStatusNumResult', 'data', 'Datas', 'UnqualifiedQuantity'], '') != 'number' ? '' : '('
                                            }${typeof SentencedToEmpty(this.props, [
                                                'serviceStatusNumResult', 'data', 'Datas', 'UnqualifiedQuantity'
                                            ], '') != 'number' ? '' : this.props.serviceStatusNumResult.data.Datas.UnqualifiedQuantity
                                            }${typeof SentencedToEmpty(this.props
                                                , ['serviceStatusNumResult', 'data', 'Datas', 'UnqualifiedQuantity'], '') != 'number' ? '' : ')'
                                            }`}</Text>
                                </View>
                                ...
                                <View style={[{
                                    ...
                                    <Text
                                        ...
                                        style={[{
                                            ...
                                            , ['serviceStatusNumResult', 'data', 'Datas', 'SubmittedQuantity'], '') != 'number' ? '' : '('
                                            }${typeof SentencedToEmpty(this.props, [
                                                'serviceStatusNumResult', 'data', 'Datas', 'SubmittedQuantity'
                                            ], '') != 'number' ? '' : this.props.serviceStatusNumResult.data.Datas.SubmittedQuantity
                                            }${typeof SentencedToEmpty(this.props
                                                , ['serviceStatusNumResult', 'data', 'Datas', 'SubmittedQuantity'], '') != 'number' ? '' : ')'
                                            }`}</Text>
                                </View>
                                ...
                                <View style={[{
                                    ...
                                    <Text
                                        ...
                                        style={[{
                                            ...
                                            , ['serviceStatusNumResult', 'data', 'Datas', 'AppealsQuantity'], '') != 'number' ? '' : '('
                                            }${typeof SentencedToEmpty(this.props, [
                                                'serviceStatusNumResult', 'data', 'Datas', 'AppealsQuantity'
                                            ], '') != 'number' ? '' : this.props.serviceStatusNumResult.data.Datas.AppealsQuantity
                                            }${typeof SentencedToEmpty(this.props
                                                , ['serviceStatusNumResult', 'data', 'Datas', 'AppealsQuantity'], '') != 'number' ? '' : ')'
                                            }`}</Text>
                                </View>
...
@connect(({ CTEquipmentPicAuditModel, CTServiceReportRectificationModel }) => ({
    stayCheckServicesListResult: CTServiceReportRectificationModel.stayCheckServicesListResult,
    stayCheckServicesList: CTServiceReportRectificationModel.stayCheckServicesList,
}))
class DefectiveList extends Component {
        ...
        if (this.props.listType != 3) {
            return (<TouchableOpacity
                onPress={() => {
                    this.props.dispatch(
                        createAction('CTServiceReportRectificationModel/updateState')({
                            serviceDescResult: { status: -1 },
                            firstLevelSelectedIndex: 0
                        }));

                    this.props.dispatch(NavigationActions.navigate({
                        routeName: 'ServiceReportRectificationDetail',
                        params: {
                            data: item
                        }
                    }));
                ...
                <View style={[{
                    ...
                    >
                        ...
                        >
                            ...
                            >{`${SentencedToEmpty(item, ['Num'], '--')}`}</Text>
                        ...
                        >
                            ...
                            >{`${SentencedToEmpty(item, ['CustomEnt'], '--')}`}</Text>
                        </View>
                        <View
                            style={[{
                                width: (SCREEN_WIDTH - 46) * 3 / 8
                    ...
                    >
                        ...
                        >{`审核人:${SentencedToEmpty(item, ['AuditUserName'], '--')}`}</Text>
                        <View style={[{ flex: 1 }]} />
                        <Text
                            numberOfLines={1}
                            style={[{
                                fontSize: 10, color: '#999999'
                                , width: (SCREEN_WIDTH - 46) * 3 / 5
                                , textAlign: 'right'
                            }]}
                        >{`审核时间:${SentencedToEmpty(item, ['AuditDate'], '--')}`}</Text>
                    </View>
                    <Image
    ...
    getPageStatus = () => {
        return SentencedToEmpty(this.props
            , ['stayCheckServicesListResult', 'status']
            , 1000);
    }

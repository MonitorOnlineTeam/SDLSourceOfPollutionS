...
export default class SevenFormViewSingle extends Component {
    ...
    getImageTitle = () => {
        if (SentencedToEmpty(this.props, ['secondItem', 'RecordId'], '') == 10) {
            return '现场勘查照片'
        } else if (SentencedToEmpty(this.props, ['secondItem', 'RecordId'], '') == 12) {
            return '验货单照片'
        }
    ...
    checkParams = () => {
        if (SentencedToEmpty(this.state, ['images'], []).length > 0) {
            ...
            ShowToast(`${this.getImageTitle()}为必填项`);
            return false;
        }
    }

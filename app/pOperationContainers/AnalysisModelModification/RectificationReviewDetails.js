...
@connect(({modelAnalysisAectificationModel}) => ({
  checkedRectificationApprovalsResult:
    modelAnalysisAectificationModel.checkedRectificationApprovalsResult,
}))
export default class RectificationReviewDetails extends Component {
  ...
  componentDidMount() {
    ...
    this.props.navigation.setOptions({
      title:
        SentencedToEmpty(
          this.props,
          ['route', 'params', 'params', 'RectificationStatus'],
          '',
        ) == 1
          ? '驳回详情'
          : '整改复核',
  ...
  statusPageOnRefresh = () => {
    ...
    this.props.dispatch(
      createAction(
        'modelAnalysisAectificationModel/getCheckedRectificationApprovals',
      )({
        params: {},
        callback: result => {
          ...
          SentencedToEmpty(
            Datas,
            ['CheckedRectificationList', 0, 'FileList'],
            [],
          ).map((item, index) => {
  ...
  renderPickedImage = () => {
    ...
    let FileList = SentencedToEmpty(
      this.props.checkedRectificationApprovalsResult,
      ['data', 'Datas', 'CheckedRectificationList', 0, 'FileList'],
      [],
    );
          ...
          <TouchableOpacity
            onPress={() => {
              this.setState({
                modalVisible: true,
                largeImageIndex: key,
                showImage: this.state.checkedRectificationListLargeImages,
              });
            }}
          ...
          <TouchableOpacity
            onPress={() => {
              this.setState({
                modalVisible: true,
                largeImageIndex: key,
                showImage: this.state.appResultLargeImages[idx],
              });
            }}
            style={[{position: 'absolute', bottom: 0, left: 0}]}>
            <View
    ...
    return (
      <StatusPage
        status={SentencedToEmpty(
          this.props,
          ['checkedRectificationApprovalsResult', 'status'],
          1000,
        )}
        //页面是否有回调按钮，如果不传，没有按钮，
        emptyBtnText={'重新请求'}

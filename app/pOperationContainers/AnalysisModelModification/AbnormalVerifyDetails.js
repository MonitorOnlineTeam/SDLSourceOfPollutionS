 ...
 * @LastEditTime: 2024-09-06 14:15:52
 * @FilePath: /SDLMainProject/app/pOperationContainers/AnalysisModelModification/AbnormalVerifyDetails.js
 */
...
// 异常核实详情
// VerifyProgress
// export default class AbnormalVerifyDetails extends Component {
//     render() {
//         return (
//             <View style={[{}]}>
//                 <Text>AbnormalVerifyDetails</Text>
//             </View>
//         )
//     }
// }
...
class AbnormalVerifyDetails extends PureComponent {
  constructor(props) {
    super(props);
    ...
    this.props.dispatch(
      createAction('alarmAnaly/GetWarningVerifyCheckInfo')({
        ...
        callback: result => {
          ...
          SentencedToEmpty(Datas, ['checkInfo', 'FileList'], []).map(
            (item, index) => {
          ...
          let appResultLargeImages = [],
            tempArray = [];
          SentencedToEmpty(Datas, ['appResult'], []).map((item, index) => {
            tempArray = [];
            SentencedToEmpty(item, ['FileList'], []).map(
  ...
  renderPickedImage = () => {
    const ProxyCode = getEncryptData();
    const rtnVal = [];
    let FileList = SentencedToEmpty(
      this.props.WarningVerifyCheckInfo,
      ['data', 'Datas', 'checkInfo', 'FileList'],
      [],
    );
    FileList.map((item, key) => {
      ...
      rtnVal.push(
        <View
          key={item.attachID}
          style={{
            width: (SCREEN_WIDTH - 130) / 3 - 5,
            height: (SCREEN_WIDTH - 130) / 3 - 5,
          }}>
          <Image
            resizeMethod={'resize'}
            source={source}
            style={{
              width: (SCREEN_WIDTH - 130) / 3 - 25,
              height: (SCREEN_WIDTH - 130) / 3 - 25,
              marginTop: 5,
              marginBottom: 5,
            }}
          />
          <TouchableOpacity
            onPress={() => {
              if (multipleImage) {
                this.setState({
                  modalVisible: true,
                  largeImageIndex: key,
                  showImage: this.state.checkInfoLargeImages,
                });
              } else {
                this.setState({
                  modalVisible: true,
                  showImage: [{url: source.uri}],
                });
              }
            }}
            ...
            <View
              style={[
                {
                  height: (SCREEN_WIDTH - 130) / 3 - 10,
                  width: (SCREEN_WIDTH - 130) / 3 - 15,
                },
              ]}
            />
          </TouchableOpacity>
            ...
            onPress={() => {
              if (multipleImage) {
                this.setState({
                  modalVisible: true,
                  largeImageIndex: key,
                  showImage: SentencedToEmpty(
                    this.state,
                    ['appResultLargeImages', idx],
                    [],
                  ),
                });
              } else {
  ...
  render() {
    let data = SentencedToEmpty(this.props.WarningVerifyCheckInfo, [
      'data',
      'Datas',
    ]);
        ...
        <ScrollView style={{flex: 1, width: SCREEN_WIDTH, padding: 13}}>
          {/* <View style={[styles.card]}>
                        ...
                        <View style={[styles.oneRow, { marginTop: 15 }]}>
                            ...
                            <SDLText style={[styles.content, { maxWidth: SCREEN_WIDTH - 140 }]}>{`${ SentencedToEmpty(data, ['WarningTypeName'], '-')
                        }`}</SDLText>
                        </View>
                        <View style={[styles.oneRow, {}]}>
                            <SDLText style={[styles.label]}>发现时间：</SDLText>
                            <SDLText style={[styles.content]}>{`${ SentencedToEmpty(data, ['WarningTime'], '-')
                }`}</SDLText>
                        </View>
                        <View style={[styles.oneRow, {}]}>
...
export default AbnormalVerifyDetails;

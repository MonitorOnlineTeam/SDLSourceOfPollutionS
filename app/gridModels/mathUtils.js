...
// js创建含数字字母的随机字符串
export const generateRdStr = () => {
    var text = '';
    var possible = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';

    for (var i = 0; i < 23; i++) text += possible.charAt(Math.floor(Math.random() * possible.length));
...
export const timeForm = (time, when) => {
    ...
    switch (when) {
        ...
        case 'hour':
            ...
            break;
        case 'second':
            let bTime = time.substring(0, 13) + ':00:00';
            mTime = moment(bTime).format('YYYY-MM-DD HH:mm');
            break;
    }
    return mTime;
};

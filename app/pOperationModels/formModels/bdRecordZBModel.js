...
export default Model.extend({
  ...
  state: {
    ...
    MainInfo: {
      ...
      WorkingDateBegin: '', //工作日期开始
      WorkingDateEnd: '', //工作日期结束
      // 本次校验开始时间
      CheckBTime: '',
      // 本次校验结束时间
      CheckETime: '',
  ...
  reducers: {
    ...
    /** 保存主表信息 */
    saveMainInfo(state, {payload: {params}}) {
    ...
    /**选择一条记录 */
    selectItem(state, {payload: {params}}) {
  ...
  effects: {
    ...
    *getForm({payload: {params}}, {call, put, update, select, take}) {
      ...
      const {taskDetail} = yield select(state => state.taskDetailModel);
      const {initPage, List} = yield select(state => state.bdRecordZBModel);
      ...
      if (result.status == 200) {
        if (SentencedToEmpty(result, ['data', 'IsSuccess'], false)) {
          ...
          let MainInfo = {
            ...data.Record.Content,
            BDRY:
              data.Record.Content.BDRY ||
              new Date().getTime() + 'BDRY'.toLowerCase(),
            BDSB:
              data.Record.Content.BDSB ||
              new Date().getTime() + 'BDSB'.toLowerCase(),
            BDJG:
              data.Record.Content.BDJG ||
              new Date().getTime() + 'BDJG'.toLowerCase(),
          }; //主表信息
          let RecordList = [];
    ...
    /** 删除表单 */
    *delForm({payload: {params}}, {call, put, update, take, select}) {
      // yield update({ liststatus: { status: -1 } });
      yield update({editstatus: {status: -1}});
      const {taskDetail} = yield select(state => state.taskDetailModel);
      const {BaseInfo} = yield select(state => state.bdRecordZBModel);
    ...
    /** 删除子表 */
    *delSubtable({payload: {params}}, {call, put, update, take, select}) {
    ...
    *delAdditionItem({payload: {params}}, {call, put, update, take, select}) {
